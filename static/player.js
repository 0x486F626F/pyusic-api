...
function reloadPlaylist() {
  ...
  playlist.forEach(function(value) {
    $("#playlist").append('<div class="music-row list-group-item"><a href="' + 
      value.audio_url + '" class="music-url">' + value.artist + ' - ' + value.title 
      + '</a></div>');
  cnt -= 1;
  if (cnt == 0) {
    tracks = $('.music-url'); 
...
$(document).ready(function() {
  player = $('#audio')[0];
  currentIndex = -1;
  getPlaylist(APIURL + '/music/search', reloadPlaylist);
...
$('#ctrl-btn-hide-add-form').click(function() {
  $('#add-music-youtube-id')[0].value = "";
  $('#add-music-title')[0].value = "";
  $('#add-music-form')[0].style.display = 'none';
});

$('#ctrl-btn-search').click(function() {
  tags = $('#search-bar-text')[0].value;
  url = APIURL + '/music/search';
  if (tags.length > 0) {
    url = url + '?tags=' + tags
  }
  getPlaylist(url, reloadPlaylist);
});

$('#ctrl-btn-add-music').click(function() {
  function addYoutubeMusic(youtubeURL, title, artist) {
    url = APIURL + '/music/info/' + youtubeURL + '?';
    if (title.length > 0) {
      url = url + '&title=' + title;
    }
    if (artist.length > 0) {
  ...
  url = $('#add-music-youtube-id')[0].value;
  title = $('#add-music-title')[0].value;
  artist = $('#add-music-artist')[0].value;
  addYoutubeMusic(url, title, artist);
  $('#add-music-youtube-id')[0].value = "";
  $('#add-music-title')[0].value = "";
  $('#add-music-artist')[0].value = "";
  $('#add-music-form')[0].style.display = 'none';
});
